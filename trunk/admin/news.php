...
<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	if ($submit_news)
	{
		if ($title==''	||	$context=='')	{echo clickback('新闻标题、新闻内容不能为空');exit;}
		else if ($html_header=='' || $html_center=='' || $html_footer=='')	{echo clickback('请选定模板');exit;}
		else
		{
			$fileroot=date($dirhtml_unit,time());
			$sql="INSERT INTO ".NEWSTABLE." (title,context,addtime,htmlroot,itfrom) VALUES ('".html($title)."','".html($context)."','".time()."','$fileroot','".html($itfrom)."')";
			$query=$db->query($sql);
			if (!$query)	{echo clickback('录入新闻失败');exit;}
			else
			{
				...
				$c_html->c_news($html_header,$html_center,$html_footer,$html_name,$dirhtml_news,$fileroot,$sql_data,1);
				update_cache('news','1');
				echo refreshback('操作成功');
				echo endhtml();
				echo wwwwanenet();
				echo showmsg('news.php','1');
				exit;
			}
		}
	}
	else if ($edit_news)
	{
		...
		else
		{
			...
			$sql="UPDATE ".NEWSTABLE." SET title='".html($title)."',context='".html($context)."',itfrom='$itfrom' WHERE id='$nid'";
			$query=$db->query($sql);
			if (!$query)	{echo clickback('编辑新闻失败');exit;}
			else
			{
				$html_name=$nid;
				$sql_data=array(
	...
	else if ($delete_news)
	{
		...
		else
		{
			...
			$sql=$db->query("select * from ".NEWSTABLE." where id in ($ids)");
			$delete_htmlfile="Delete html files result ";
			while ($row=$db->row($sql))
			{
				$delete_htmlfile.=$c_html->d_news($row['htmlroot'],$row['id'],1);
			}
			$query_sql=$db->query("delete from ".NEWSTABLE." where id in ($ids)");
	...
	else if ($tohtml_news)
	{
		...
		else
		{
			...
			while ($row=$db->row($sql))
			{
				...
				$c_html->c_news($html_header,$html_center,$html_footer,$row['id'],$dirhtml_news,$row['htmlroot'],$sql_data,1);
			}
			update_cache('news','1');
			echo refreshback('操作成功');
			echo endhtml();
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="lrd">
      <tr>
        <td height="22" align="center" background="images/admin_tablebar.gif">[ + <a href="news.php">新闻管理</a> + ]&nbsp;&nbsp;&nbsp;[ + <a href="news.php?action=add">添 加 新 闻</a> + ]</td>
      </tr>
      <tr>
		...
		<?
			if ($action=='edit')
			{
				$sql=$db->query("select * from ".NEWSTABLE." where id='$info'");
				$row=$db->row($sql);
			?>
			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1">
			<form action="news.php" method="post" name="input">
			  <tr bgcolor="#F1F2F4">
			  ...
			  <tr bgcolor="#F1F2F4">
				<td width="20%" height="25" align="center">新闻内容</td>
			  ...
			  <tr bgcolor="#F1F2F4">
				...
				<td height="25"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td height="25">&nbsp;&nbsp;模板主体&nbsp;:&nbsp;<?=$HTML_TPL->get_center($default_news)?></td>
                  </tr>
                  <tr>
			  ...
			  <tr bgcolor="#F1F2F4">
				<td height="30" colspan="2" align="center"><input name="edit_news" type="submit" class="input" id="edit_news" value="编辑新闻">
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				  <input name="Submit" type="reset" class="input" value="重置新闻内容"></td>
				</tr>
			</form>
			</table>
			<? }
			...
			<form action="news.php" method="post" name="input">
			  ...
			  <tr bgcolor="#F1F2F4">
				<td height="30" colspan="2" align="center"><input name="submit_news" type="submit" class="input" id="submit_news" value="提交新闻">
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				  <input name="Submit" type="reset" class="input" value="重置新闻内容"></td>
				</tr>
			</form>
			...
			<FORM action="news.php" method="post" name="input">
			  ...
			  <?
					...
					while ($row=$db->row($sql))
					{
						$htmlfile='../'.$htmlroot.$dirhtml_news.'/'.$row['htmlroot'].'/'.$row['id'].'.html';
					?>
					  <tr bgcolor="#F1F2F4">
						...
						<td align="center"><a href="news.php?action=edit&info=<?=$row['id']?>">编辑</a></td>
			  ...
			  <tr bgcolor="#F1F2F4">
			    ...
			    <td align="left"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td height="22">主体&nbsp;:&nbsp;                      <?=$HTML_TPL->get_center($default_news)?></td>
                  </tr>
                  <tr>
			    ...
			    <td colspan="2" align="center"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td height="30" align="center"><input name="delete_news" type="submit" class="input" id="delete_news" value=" 删 除 新 闻 "></td>
                  </tr>
                  <tr>
                    <td height="30" align="center"><input name="tohtml_news" type="submit" class="input" id="tohtml_news" value="批量生成 html"></td>
                  </tr>
                  <tr>
                    <td height="5"></td>
                  </tr>
