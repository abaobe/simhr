...
<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	if ($submit_joblaw)
	{
		if ($title==''	||	$context=='')	{echo clickback('标题、政策法规内容不能为空');exit;}
		else if ($html_header=='' || $html_center=='' || $html_footer=='')	{echo clickback('请选定模板');exit;}
		else
		{
			$fileroot=date($dirhtml_unit,time());
			$sql="INSERT INTO ".LAWTABLE." (itfrom,title,context,addtime,htmlroot) VALUES ('".html($itfrom)."','".html($title)."','".html($context)."','".time()."','$fileroot')";
			$query=$db->query($sql);
			if (!$query)	{echo clickback('录入政策法规失败');exit;}
			else
			{
				...
				$c_html->c_joblaw($html_header,$html_center,$html_footer,$html_name,$dirhtml_law,$fileroot,$sql_data,1);
				update_cache('law','1');
				echo refreshback('操作成功');
				echo endhtml();
				echo wwwwanenet();
				echo showmsg('job_law.php','1');
				exit;
			}
		}
	}
	...
	{
		if ($title==''	||	$context=='')	{echo clickback('政策法规标题、政策法规内容不能为空');exit;}
		else if ($html_header=='' || $html_center=='' || $html_footer=='')	{echo clickback('请选定模板');exit;}
		else
		{
			$fileroot=$editroot	?	$editroot	:	date($dirhtml_unit,time());
			$sql="UPDATE ".LAWTABLE." SET title='".html($title)."',context='".html($context)."',itfrom='$itfrom' WHERE id='$nid'";
			$query=$db->query($sql);
			if (!$query)	{echo clickback('编辑政策法规失败');exit;}
			else
			{
				$html_name=$nid;
				$sql_data=array(
	...
	else if ($delete_joblaw)
	{
		...
		else
		{
			...
			$sql=$db->query("select * from ".LAWTABLE." where id in ($ids)");
			$delete_htmlfile="Delete html files result ";
			while ($row=$db->row($sql))
			{
				$delete_htmlfile.=$c_html->d_joblaw($row['htmlroot'],$row['id'],1);
			}
			$query_sql=$db->query("delete from ".LAWTABLE." where id in ($ids)");
	...
	else if ($tohtml_joblaw)
	{
		...
		else
		{
			...
			while ($row=$db->row($sql))
			{
				...
				$c_html->c_joblaw($html_header,$html_center,$html_footer,$row['id'],$dirhtml_law,$row['htmlroot'],$sql_data,1);
			}
			update_cache('law','1');
			echo refreshback('操作成功');
			echo endhtml();
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="lrd">
      <tr>
        <td height="22" align="center" background="images/admin_tablebar.gif">[ + <a href="job_law.php">政策法规管理</a> + ]&nbsp;&nbsp;&nbsp;[ + <a href="job_law.php?action=add">添加政策法规</a> + ]</td>
      </tr>
      <tr>
		...
		<?
			if ($action=='edit')
			{
				$sql=$db->query("select * from ".LAWTABLE." where id='$info'");
				$row=$db->row($sql);
			?>
			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1">
			<form action="job_law.php" method="post" name="input">
			  <tr bgcolor="#F1F2F4">
				<td width="20%" height="25" align="center">政策法规标题</td>
			  ...
			  <tr bgcolor="#F1F2F4">
				<td width="20%" height="25" align="center">政策法规内容</td>
			  ...
			  <tr bgcolor="#F1F2F4">
				...
				<td height="25"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td height="25">&nbsp;&nbsp;模板主体&nbsp;:&nbsp;<?=$HTML_TPL->get_center($default_law)?></td>
                  </tr>
                  <tr>
			  ...
			  <tr bgcolor="#F1F2F4">
				<td height="30" colspan="2" align="center"><input name="edit_joblaw" type="submit" class="input" id="edit_joblaw" value="编辑政策法规">
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				  <input name="Submit" type="reset" class="input" value="重置政策法规内容"></td>
				</tr>
			</form>
			</table>
			<? }
			...
			<form action="job_law.php" method="post" name="input">
			  ...
			  <tr bgcolor="#F1F2F4">
				<td height="30" colspan="2" align="center"><input name="submit_joblaw" type="submit" class="input" id="submit_joblaw" value="提交政策法规">
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				  <input name="Submit" type="reset" class="input" value="重置政策法规内容"></td>
				</tr>
			</form>
			...
			<FORM action="job_law.php" method="post" name="input">
			  <tr bgcolor="#E1E3E8">
				...
				<td height="25" align="center">政策法规标题</td>
			  ...
			  <?
					...
					while ($row=$db->row($sql))
					{
						$htmlfile='../'.$htmlroot.$dirhtml_law.'/'.$row['htmlroot'].'/'.$row['id'].'.html';
					?>
					  <tr bgcolor="#F1F2F4">
						...
						<td align="center"><a href="job_law.php?action=edit&info=<?=$row['id']?>">编辑</a></td>
			  ...
			  <tr bgcolor="#F1F2F4">
			    ...
			    <td align="left"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td height="22">主体&nbsp;:&nbsp;                      <?=$HTML_TPL->get_center($default_law)?></td>
                  </tr>
                  <tr>
			    ...
			    <td colspan="2" align="center"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td height="30" align="center"><input name="delete_joblaw" type="submit" class="input" id="delete_joblaw" value="删除政策法规 "></td>
                  </tr>
                  <tr>
                    <td height="30" align="center"><input name="tohtml_joblaw" type="submit" class="input" id="tohtml_joblaw" value="批量生成 html"></td>
                  </tr>
                  <tr>
                    <td height="5"></td>
                  </tr>
