<?php
	...
	*/
	...
	require 'globals.php';
	tpl_load('jobway');
	$headtitle=headtitle($webtitle.' 求职攻略');
	require_once 'common/common.php';
	$count=$num_jobwayphp_list;
	$table=WAYTABLE;
	if ($action=='search')
	{
	...
	while ($new=$db->row($query))
	{
		$tpl->set_var(
			array(
					...
					'NEWS_LINK'		=>	$htmlroot.$dirhtml_way.'/'.$new['htmlroot'].'/'.$new['id'].'.html',
					'DISCUSS'		=>	$new[replies],
			)
		);
		$tpl->parse('ways','way',true);
	}
